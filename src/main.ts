...
function doOn() {
  ...
  im.on(Im.event.IM_TOGGLE, () => {
    println('need to toggle');
  });
  im.on(Im.event.IM_MSG, (custom, msg) => {
    console.log(custom);
    println('have msg');
//    im.setCurrSession(Im.constant.SCENE_P2P, msg.from);
  });
...
const options = {
  ...
  isOnlinCustomer: (scene, to) => {
    return scene === Im.constant.SCENE_P2P && to === '123106';
  },
...
$('#setCurrSession').on('click', () => {
  im.setCurrSession(Im.constant.SCENE_P2P, '083456');
});
$('#destroy').on('click', () => {
  im.destroy();
});
